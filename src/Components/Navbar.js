...
import {navLinks} from '../commondata/common'
    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`}>
      <div className='w-full flex justify-between items-center max-w-7xl mx-auto'>
          <Link to="/" className='flex items-center gap-2' onClick={
            ...
            <p className='text-white-[18px] font-bold cursor-pointer'>Chithiraivel <span className='sm:block hidden'>javascript</span></p>
            </Link>
            ...
            <div className='sm:hidden flex flex-1 justify-end items-center'>
                <h1 onClick={()=>setToggle(!toggle)} className='w-[28px]  h-[28px] object-contain cursor-pointer'>{toggle? '==' : 'x'}</h1>
            </div>
